	...
	opts = function()
		...
		return {
			opts = {
				disable_winbar_cb = function(args)
					return status.condition.buffer_matches({
						...
						filetype = {
							"NvimTree", "neo%-tree", "dashboard", "Outline",
							"aerial"
						}
					}, args.buf)
				end
			...
			tabline = {       -- bufferline
				{
					...
					condition = function(self)
						...
						return status.condition.buffer_matches({
							filetype = {
								"aerial", "dapui_.", "neo%-tree", "NvimTree"
							}
						}, vim.api.nvim_win_get_buf(self.winid))
					end,
					provider = function(self)
						return string.rep(" ", vim.api
							.nvim_win_get_width(self.winid) +
							1)
					end,
					hl = { bg = "tabline_bg" }
				},
				...
				status.component.fill { hl = { bg = "tabline_bg" } },            -- fill the rest of the tabline with background color
				{
					...
					status.heirline.make_tablist {           -- component for each tab
						...
						hl = function(self)
							return status.hl.get_attributes(status.heirline
								.tab_type(self,
									"tab"),
								true)
						end
			...
			statuscolumn = vim.fn.has "nvim-0.9" == 1 and {
						status.component.foldcolumn(), status.component.fill(),
						status.component.numbercolumn(), status.component.signcolumn()
					} or nil
		}
	end,
