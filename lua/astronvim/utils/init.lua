...
function M.get_icon(kind)
  ...
  if not M[icon_pack] then
    M.icons = astronvim.user_opts("icons",
      require "astronvim.icons.nerd_font")
    M.text_icons = astronvim.user_opts("text_icons",
      require "astronvim.icons.text")
  end
...
function M.system_open(path)
  ...
  elseif vim.fn.has "unix" == 1 and vim.fn.executable "xdg-open" == 1 then
    cmd = { "xdg-open" }
  elseif (vim.fn.has "mac" == 1 or vim.fn.has "unix" == 1) and
      vim.fn.executable "open" == 1 then
    cmd = { "open" }
  end
  if not cmd then
    M.notify("Available system opening tool not found!", "error")
  end
  vim.fn.jobstart(vim.fn.extend(cmd, { path or vim.fn.expand "<cfile>" }),
    { detach = true })
end
...
function M.cmd(cmd, show_error)
  ...
  return success and
      result:gsub("[\27\155][][()#;?%d]*[A-PRZcf-ntqry=><~]", "") or
      nil
end
