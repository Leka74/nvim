...
M.env.buf_matchers = {
  ...
  bufname = function(pattern_list, bufnr)
    return pattern_match(vim.fn.fnamemodify(
        vim.api.nvim_buf_get_name(bufnr or 0), ":t"),
      pattern_list)
  end
}
...
for _, sign in ipairs {
  "Topdelete", "Untracked", "Add", "Changedelete", "Delete"
} do
...
M.env.sign_handlers = astronvim.user_opts("heirline.sign_handlers",
  M.env.sign_handlers)
...
function M.hl.lualine_mode(mode, fallback)
  local lualine_avail, lualine = pcall(require,
    "lualine.themes." .. vim.g.colors_name)
  local lualine_opts = lualine_avail and lualine[mode]
  return lualine_opts and type(lualine_opts.a) == "table" and
      lualine_opts.a.bg or fallback
end
...
function M.hl.filetype_color(self)
  ...
  local _, color = devicons.get_icon_color(
    vim.fn.fnamemodify(vim.api.nvim_buf_get_name(self and
        self.bufnr or
        0),
      ":t"), nil, { default = true })
  return { fg = color }
...
function M.hl.file_icon(name)
  ...
  return function(self)
    if hl_enabled == true or
        (type(hl_enabled) == "function" and hl_enabled(self)) then
...
function M.init.breadcrumbs(opts)
  ...
  return function(self)
    ...
    if opts.max_depth and opts.max_depth > 0 then
      ...
      if start_idx > 0 then
        table.insert(children, {
          provider = require("astronvim.utils").get_icon "Ellipsis" ..
              opts.separator
        })
      end
    end
    ...
    for i, d in ipairs(data) do
      if i > start_idx then
        local child = {
          {
            provider = string.gsub(d.name, "%%", "%%%%"):gsub(
              "%s*->%s*", "")
          },           -- add symbol name
          on_click = {
        ...
        if opts.icon.enabled then         -- add icon and highlight if enabled
          ...
          table.insert(child, 1, {
            ...
            hl = (hl and vim.fn.hlexists(hlgroup) == 1) and hlgroup or
                nil
          })
        end
...
function M.provider.numbercolumn(opts)
  ...
  return function()
    ...
    local str = ((num and not relnum) and "%l") or
        ((relnum and not num) and "%r") or
...
function M.provider.foldcolumn(opts)
  ...
  return function()                                                -- move to M.provider.fold_indicator
    ...
    local foldinfo = width > 0 and ffi.C.fold_info(wp, vim.v.lnum) or
        { start = 0, level = 0, llevel = 0, lines = 0 }
    ...
    if width ~= 0 then
      ...
      if foldinfo.level == 0 then
        ...
        local first_level =
            foldinfo.level - width - (closed and 1 or 0) + 1
        if first_level < 1 then first_level = 1 end

        for col = 1, width do
          str = str ..
              (((closed and
              (col == foldinfo.level or col == width)) and
              foldclosed) or
              ((foldinfo.start == vim.v.lnum and first_level +
              col > foldinfo.llevel) and foldopen) or
              foldsep)
...
function M.provider.tabnr()
  return function(self)
    return
        (self and self.tabnr) and "%" .. self.tabnr .. "T " .. self.tabnr ..
        " %T" or ""
  end
end
...
function M.provider.spell(opts)
  opts = extend_tbl({
    str = "",
    icon = { kind = "Spellcheck" },
    show_empty = true
  }, opts)
  return
      function()
        return M.utils.stylize(vim.wo.spell and opts.str, opts)
      end
end
...
function M.provider.paste(opts)
  opts = extend_tbl({ str = "", icon = { kind = "Paste" }, show_empty = true },
    opts)
  return function()
    return M.utils.stylize(vim.opt.paste:get() and opts.str, opts)
  end
end
...
function M.provider.search_count(opts)
  local search_func = vim.tbl_isempty(opts or {}) and
      function() return vim.fn.searchcount() end or
      function() return vim.fn.searchcount(opts) end
  return function()
    local search_ok, search = pcall(search_func)
    if search_ok and type(search) == "table" and search.total then
      return M.utils.stylize(string.format("%s%d/%s%d", search.current >
          search.maxcount and ">" or
          "", math.min(
            search.current,
            search.maxcount),
...
function M.provider.mode_text(opts)
  local max_length = math.max(unpack(vim.tbl_map(function(str)
    return #str[1]
  end, vim.tbl_values(M.env.modes))))
  return function()
    ...
    if opts.pad_text then
      ...
      elseif opts.pad_text == "center" then
        text = string.rep(" ", math.floor(padding / 2)) .. text ..
...
function M.provider.percentage(opts)
  opts = extend_tbl({ escape = false, fixed_width = true, edge_text = true },
    opts)
  return function()
    local text = "%" ..
...
function M.provider.ruler(opts)
  ...
  local padding_str = string.format("%%%dd:%%-%dd", opts.pad_ruler.line,
    opts.pad_ruler.char)
  return function()
...
function M.provider.filename(opts)
  ...
  return function(self)
    local filename = vim.fn.fnamemodify(
      opts.fname(self and self.bufnr or 0), opts.modify)
    return M.utils.stylize((filename == "" and opts.fallback or filename),
      opts)
  end
end
  ...
  return function(self)
    ...
    for _, value in ipairs(vim.t.bufs) do
      if name == opts.buf_name(value) and value ~= opts.bufnr then
        local other = {}
        for match in (vim.api.nvim_buf_get_name(value) .. "/"):gmatch(
          "(.-)" .. "/") do
    ...
    return M.utils.stylize((opts.max_length > 0 and #unique_path >
      opts.max_length and
      string.sub(unique_path, 1,
        opts.max_length - 2) ..
      get_icon "Ellipsis" .. "/") or unique_path,
      opts)
  end
end
...
function M.provider.file_modified(opts)
  ...
  return function(self)
    return M.utils.stylize(
      M.condition.file_modified((self or {}).bufnr) and opts.str,
      opts)
  end
end
...
function M.provider.file_read_only(opts)
  opts = extend_tbl({
    str = "",
    icon = { kind = "FileReadOnly" },
    show_empty = true
  }, opts)
  return function(self)
    return M.utils.stylize(
      M.condition.file_read_only((self or {}).bufnr) and opts.str,
      opts)
  end
end
...
function M.provider.git_branch(opts)
  return function(self)
    return M.utils.stylize(vim.b[self and self.bufnr or 0].gitsigns_head or
      "", opts)
  end
end
...
function M.provider.git_diff(opts)
  ...
  return function(self)
    ...
    return M.utils.stylize(status and status[opts.type] and
      status[opts.type] > 0 and
      tostring(status[opts.type]) or "", opts)
  end
end
...
function M.provider.diagnostics(opts)
  ...
  return function(self)
    ...
    local count = #vim.diagnostic.get(bufnr, opts.severity and
      {
        severity = vim.diagnostic.severity[opts.severity]
      })
...
function M.provider.lsp_progress(opts)
  return function()
    ...
    return M.utils.stylize(Lsp and (get_icon("LSP" ..
        ((Lsp.percentage or 0) >=
        70 and
        {
          "Loaded", "Loaded", "Loaded"
        } or { "Loading1", "Loading2", "Loading3" })[math.floor(
          vim.loop.hrtime() /
          12e7) % 3 + 1]) ..
      (Lsp.title and " " .. Lsp.title or "") ..
      (Lsp.message and " " .. Lsp.message or "") ..
      (Lsp.percentage and " (" .. Lsp.percentage ..
      "%)" or "")), opts)
  end
end
...
function M.provider.lsp_client_names(opts)
  ...
  return function(self)
    ...
    for _, client in pairs(vim.lsp.get_active_clients {
      bufnr = self and self.bufnr or 0
    }) do
      if client.name == "null-ls" and opts.expand_null_ls then
        local null_ls_sources = {}
        for _, type in ipairs { "FORMATTING", "DIAGNOSTICS" } do
          for _, source in ipairs(
            M.utils.null_ls_sources(vim.bo.filetype,
              type)) do
...
function M.condition.is_git_repo(bufnr)
  ...
  return vim.b[bufnr or 0].gitsigns_head or
      vim.b[bufnr or 0].gitsigns_status_dict
end
...
function M.condition.git_changed(bufnr)
  ...
  return git_status and (git_status.added or 0) + (git_status.removed or 0) +
      (git_status.changed or 0) > 0
end
...
function M.condition.has_filetype(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
  return vim.fn.empty(vim.fn.expand "%:t") ~= 1 and
      vim.bo[bufnr or 0].filetype and vim.bo[bufnr or 0].filetype ~= ""
end
...
function M.condition.treesitter_available(bufnr)
  ...
  return parsers.has_parser(parsers.get_buf_lang(bufnr or
    vim.api
...
function M.utils.stylize(str, opts)
  ...
  return str and (str ~= "" or opts.show_empty) and opts.separator.left ..
      M.pad_string(icon .. (opts.escape and escape(str) or str),
        opts.padding) .. opts.separator.right or ""
end
...
function M.component.file_info(opts)
  ...
  return M.component.builder(M.utils.setup_providers(opts, {
    "file_icon", "unique_path", "filename", "filetype", "file_modified",
    "file_read_only", "close_button"
  }))
end
...
function M.component.tabline_file_info(opts)
  return M.component.file_info(extend_tbl({
    ...
    unique_path = {
      hl = function(self)
        return M.hl.get_attributes(self.tab_type .. "_path")
      end
    },
    close_button = {
      hl = function(self)
...
function M.component.cmd_info(opts)
  opts = extend_tbl({
    ...
    surround = {
      ...
      color = "cmd_info_bg",
      condition = function()
        return M.condition.is_hlsearch() or
            M.condition.is_macro_recording()
      end
    },
...
function M.component.signcolumn(opts)
  opts = extend_tbl({
    ...
    on_click = {
      ...
      callback = function(...)
        local args = M.utils.statuscolumn_clickargs(...)
        if args.sign and args.sign.name and
            M.env.sign_handlers[args.sign.name] then
...
function M.component.builder(opts)
  ...
  for key, entry in pairs(opts) do
    if type(key) == "number" and type(entry) == "table" and
        M.provider[entry.provider] and
        (entry.opts == nil or type(entry.opts) == "table") then
  ...
  return opts.surround and
      M.utils
      .surround(opts.surround.separator, opts.surround.color,
        children, opts.surround.condition) or children
end
...
function M.utils.width(is_winbar)
  return vim.o.laststatus == 3 and not is_winbar and vim.o.columns or
...
function M.pad_string(str, padding)
  padding = padding or {}
  return str and str ~= "" and string.rep(" ", padding.left or 0) .. str ..
      string.rep(" ", padding.right or 0) or ""
end
...
function M.utils.surround(separator, color, component, condition)
  ...
  separator = type(separator) == "string" and M.env.separators[separator] or
      separator
...
function M.utils.null_ls_sources(filetype, method)
  ...
  return methods_avail and
      M.utils.null_ls_providers(filetype)[methods.internal[method]] or
      {}
end
...
function M.utils.statuscolumn_clickargs(self, minwid, clicks, button, mods)
  ...
  args.char = vim.fn.screenstring(args.mousepos.screenrow,
    args.mousepos.screencol)
  if args.char == " " then
    args.char = vim.fn.screenstring(args.mousepos.screenrow,
...
M.heirline.make_buflist = function(component)
  ...
  return require("heirline.utils").make_buflist(
    M.utils.surround("tab", function(self)
      return {
      ...
      -- bufferlist
      init = function(self)
        self.tab_type = M.heirline.tab_type(self)
      end,
      on_click = {
